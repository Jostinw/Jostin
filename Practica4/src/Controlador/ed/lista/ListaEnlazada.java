...
public class ListaEnlazada<E> {
     ...
     */
//    public void toList(Object[] array) {
//    NodoLista<E> nodoActual = cabecera;
//    int index = 0;
//
//    while (nodoActual != null && index < array.length) {
//        array[index] = nodoActual.getInfo();
...
//    }
   ...
   public void quicksort(E[] arr, int derecha, int izquierda, Integer tipo) {
        if (derecha < izquierda) {
            int pi = particion(arr, derecha, izquierda, tipo);

            quicksort(arr, derecha, pi - 1, tipo);
            quicksort(arr, pi + 1, izquierda, tipo);
        }
    }

    public int particion(E[] arr, int izquierda, int derecha, Integer tipo) {
        E pivote = arr[derecha];
        int i = (izquierda - 1);
        for (int j = izquierda; j < derecha; j++) {
            if (tipo == 0) {
                if ((Integer) (arr[j]) <= ((Integer) pivote)) {
                    i++;
                    E temp = arr[i];
                    arr[i] = arr[j];
                    arr[j] = temp;
